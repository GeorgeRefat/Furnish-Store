...
export class SignupComponent {
  ...
  constructor(private ApiuserService : ApiuserService , private router : Router ,private authService: AuthService , private route: ActivatedRoute) {}
  ...
  async checkEmailUnique(email: string): Promise<boolean> {
    ...
    try {
        const response = await firstValueFrom(this.ApiuserService.checkEmailUnique(email));
  ...
  async onSubmit() {
    if (this.username.trim() === '' || this.username.length < 3) {
      this.showToastMessage('Please, enter your username with at least 3 characters...');
      return;
    }
    ...
    if (!isUnique) {
        this.showToastMessage('This email is already registered. Please use a different email.');
        return;
    }
    ...
    this.ApiuserService.registerUser(userData).subscribe(
        ...
        error => {
          ...
          if (error.error && error.error.errors) {
              error.error.errors.forEach((err: { msg: string }) => console.log(err.msg));
          } else {
  ...
  //   const isUniqueEmail = await this.checkEmailUnique(this.email);
  //   if (!isUniqueEmail) {
  //     this.showToastMessage('This email is already registered. Please use a different email.');
  //     return false;
  //   }
